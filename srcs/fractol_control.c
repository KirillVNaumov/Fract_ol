...
int				motion_hook(int x, int y, t_fractol *fractol)
{
  ft_printf("In motion_hook\n");
	...
	// 		&& (x < WIN_WIDTH && y < WIN_HEIGHT))
	// {
		fractol->offset.x = fractol->offset.x + fractol->mouse.start.x - /*fractol->mouse.pos.x*/x;
		fractol->offset.y = fractol->offset.y + fractol->mouse.start.y - /*fractol->mouse.pos.y*/y;
	// }
	return (0);
}